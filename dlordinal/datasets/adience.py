...
# python partition_retinopathy.py <path to trainLabels.csv> <path to train images folder> 
# <path to folder to contain transformed images> <path to folder to contain all partitions>
...
class Adience():
    def __init__(
        ...
        ranges: list = [(0, 2), (4, 6), (8, 13), (15, 20), (25, 32), (38, 43), (48, 53), (60, 100)],
        test_size: float = 0.2,
        extract: bool = True,
        transfrom: bool = True,
    ) -> None:
    
        # super(Adience, self).__init__(folds_path, images_path, transformed_images_path, partition_path)
        super().__init__()
        ...
        # self.transformed_images_path = transformed_images_path
        ...
        folds = [pd.read_csv(self.folds_path / f'fold_{f}_data.txt', sep='\t') for f in range(5)]
    ...
    def process_and_split(self, folds: list) -> None:
        ...
        for f, fold in enumerate(folds):
            ...
            fold_dfs.append(pd.DataFrame(dict(path=fold.apply(self.image_path_from_row, axis='columns'), age=fold['age'])))
        ...
        for partition in range(self.number_partitions):
            for c in range(len(self.ranges)):
                (self.partition_path / f'{partition}/train/{c}').mkdir(parents=True, exist_ok=True)
                (self.partition_path / f'{partition}/test/{c}').mkdir(parents=True, exist_ok=True)
                
        sss = StratifiedShuffleSplit(self.number_partitions, test_size=self.test_size, random_state=0)
        for partition, (train_index, test_index) in tqdm(enumerate(sss.split(df, df['age']))):
            ...
            for name, partition_df in zip(('train', 'test'), (train_df, test_df)):
                for row in tqdm(partition_df.itertuples(), total=len(partition_df), leave=False, desc=name):
                    ...
                    new_path = self.partition_path / f'{partition}/{name}/{row.age}/{image_path.name}'
